...
//clonx	public TrackManager trackManager;
//clonx	public Character character;
//clonx	public CharacterCollider characterCollider;
...
//clonx	public Consumable inventory;
...
//clonx	public List<Consumable> consumables { get { return m_ActiveConsumables; } }
    ...
    //clonx protected List<Consumable> m_ActiveConsumables = new List<Consumable>();
...
//		m_Animator = GetComponent<Animator>();
	...
	public void Begin()
	{
//clonx        character.animator.SetBool(s_DeadHash, false);

//clonx		characterCollider.Init ();

//clonx		m_ActiveConsumables.Clear();
	}

	public void End()
	{
//clonx        CleanConsumable();
    }
/*clonx
...
*/        
	...
	protected void Update ()
    {
        ...
        {
//			testForward+=1;
//			if (testForward>1) testForward=1;
            Jump();
        }
		else if (Input.GetKeyDown(KeyCode.DownArrow))
		{
			testForward-=1;
			if (testForward<0) testForward=1;
		...
		Vector3 verticalTargetPosition = new Vector3 (m_TargetPosition.x, characterCollider.transform.localPosition.y, characterCollider.transform.localPosition.z);
/*clonx
		if (m_Sliding)
		{
        ...
        if(m_Jumping)
        {
			if (trackManager.isMoving)
			{
				...
				{
					m_Jumping = false;
//clonx					character.animator.SetBool(s_JumpingHash, false);
				}
				else
				{
...
//		if (testForward>0)
//			characterCollider.transform.localPosition = Vector3.MoveTowards(characterCollider.transform.localPosition, new Vector3(characterCollider.transform.localPosition.x,characterCollider.transform.localPosition.y,characterCollider.transform.localPosition.z+moveSpeed), moveSpeed * Time.deltaTime); //clonx

		if (blobShadow != null) { //clonx
...
//clonx            character.animator.SetFloat(s_JumpingSpeedHash, animSpeed);
//clonx            character.animator.SetBool(s_JumpingHash, true);
//clonx			m_Audio.PlayOneShot(character.jumpSound);
			m_Jumping = true;
        }
*/        
    }
...
//clonx			character.animator.SetFloat(s_JumpingSpeedHash, animSpeed);
//clonx			character.animator.SetBool(s_SlidingHash, true);
			m_Audio.PlayOneShot(slideSound);
			m_Sliding = true;

//clonx			characterCollider.Slide(true);
		}
*/		
	}
	...
	public void StopSliding()
	{
		if (m_Sliding)
		{
//clonx			character.animator.SetBool(s_SlidingHash, false);
			m_Sliding = false;

//clonx			characterCollider.Slide(false);
		}
	}

	private void FixedUpdate()
	//clonx 14.09.2018 from ThirdPersonUserControl
	{
		Vector3 move=new Vector3(0,0,testForward);	
		m_Character.Move(move, false, m_Jumping);
		m_Jumping = false;
		//UpdateAnimator (move);
...
*/
		...
		if (targetLane < minLine || targetLane > maxLine)
            // Ignore, we are on the borders.
            return;

        m_CurrentLane = targetLane;
		m_TargetPosition = new Vector3((m_CurrentLane - startLine) * laneOffset,0,1 );//trackManager.laneOffset, 0, 0);
    }
